...
// Driver Signup
async function DriverSignup(req, res, next) {
  try {
    req.body.role = "Driver";
    return signup(req, res, next);
  } catch (err) {
    console.error("Error in Driver signup:", err);
    return res
      .status(400)
      .json({ message: "Error in Driver signup", error: err.message });
  }
}
...
async function resendVerificationCode(req, res) {
  try {
    ...
    await sendVerificationCodeEmail(req, user.email, verificationCode);
